...
```{r visualization, fig.cap = "Data distribution \\label{fig1_normal}"}
    library(ggplot2)
    library(reshape2)
...
>  The test used is the difference test, which is used to evaluate the performance on a group of subjects after changing the conditions, as it is done in the Stroop experiment. The steps are the following @stanford_t_test:

>1. Calculate the difference $d= t_i-t_c$
>2. Calculate the mean of the difference $\bar{d}=$ `r round(mu_diff, 3)` 
>3. Calculate the standard error $SE(\bar{d})= \frac{s_d}{\sqrt{n}}$, for $s_d$ the standard deviation of the differences. $SE(\bar{d})= \frac{s_d}{\sqrt{n}}=$ `r round(SE, digits = 3)`
>4. Calculate the t-statistic: $t=\frac{\bar{d}}{SE(\bar{d})}=$ `r round(t,digits=3)`. Under the null distribution this statistic follows a t-distribution with n-1 degrees of freedom.
>5.  See where t falls in the $t_{n-1}$ distribution Get the value fron the table: this will give the p-value for the paired t-test.`r t_critical=qt(0.95,dfr)` $t_{critical}=$ `r round(t_critical,3)`
...
> $\bar{d} \pm t^* \frac{s_d}{\sqrt{n}}=$ [`r round(mu_diff+ qt(0.025,dfr)*SE,3)`, `r round(mu_diff- qt(0.025,dfr)*SE,3)`] 
...
http://www.cogsci.bme.hu/~ktkuser/KURZUSOK/BMETE47MC38/2015_2016_1/7_The%20t-test.pdf  

<!-- >
Independent samples!
### **Standard error**

> It can be considered that this test is performed on independent samples, as *congruent* and *incongruent* cases have different independent parameters. the **standard error** will be
...
> The dregrees of freedom for two independen group of samples can be expressed as : $df= n_1 + n_2 - 2=$ `r dfr`.
