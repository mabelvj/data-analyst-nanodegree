...
require(ggpairs)
require(data.table)
tmp <- data.table(redwines, keep.rownames=TRUE, keep.colnames= TRUE)

ggpairs(data=tmp, diag=list(continuous="density"),
        lower= list(continuous = wrap("points", alpha = 1/5)), columns=c("sulphates","pH","total.sulfur.dioxide"), mapping = ggplot2::aes(color = quality.cat), axisLabels="show")
```
