...
From the general correlation matrix (fig.\@ref(fig:MultivariateCorrelation)), three main variables can be selected due to their high correlation with quality: **alcohol** ($\rho=0.5$), **volatile.acidity** ($\rho=0.4$) and  **sulphates** ($\rho=0.3$). In order to see if they are suitable to perform an analysis let us explore the correlation between them and also the distribution of samples in bivariate plots in figure \@ref(fig:corrreport).
...
For the three main variables (fig.\@ref(fig:lmquality)) influencing the quality of wine:

- **sulphates vs quality** 
  The correlation between this two variables is $\rho=0.3$.
- **pH vs quality**
  The correlation between this two variables is negative with a value $\rho=-0.1$.
- **total sulfur dioxide vs quality**
  The correlation between this two variables is also negative with a value $\rho=-0.2$.
...
Figure \@ref(fig:bivariateq1) shows the relationship between fixed acidity and density, and between fixed acidity and pH. Fixed acidity and density have a strong positive correlation ($\rho=0.7$) and fixed acidity and pH show a strong negative correlation ($\rho=-0.7$).
...
- For the case of sulphates-total sulfur dioxide there seems to be no dependence,  what is coincident with the value of the correlation for these two variables $\rho=0$.

- For the case of pH-total sulfur dioxide there seems to be no dependence,  what is coincident with the value of the correlation for these two variables $\rho=-0.1$.
...
- For the case of fixed acidity vs pH there is a negative dependence, coincident with the value of the correlation for these two variables $\rho=-0.7$.

- For the case of fixed acidity vs citric acid  there seems to be a strong dependence,  what is coincident with the value of the correlation for these two variables $\rho=0.7$.

- For the case of volatile acidity vs citric acid there seems to be no dependence,  what is coincident with the value of the correlation for these two variables $\rho=-0.6$.
...
redwines$quality.error <- (redwines$quality.predictions - redwines$quality)/redwines$quality
...
### Plot Three
```{r echo=FALSE, PlotThree ,fig.width=8, fig.asp=0.5,  fig.align= 'center', fig.cap=" Citric acidity vs quality"}
