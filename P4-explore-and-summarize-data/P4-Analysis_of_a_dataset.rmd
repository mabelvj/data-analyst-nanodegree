...
corr1 <-  ggcorr(data=subset(redwines, quality.cat=="bad", select = cols_names),
       label = TRUE,
       geom = "blank",
       hjust = 1,
       angle = -45,
...
r2 <- ggplot2::ggcorr(data=subset(redwines, quality.cat=="medium"),
       label = TRUE,
       label_alpha = TRUE,
       name = "") +
  ggplot2::theme(legend.position = "bottom")

r3 <- ggplot2::ggcorr(data=subset(redwines, quality.cat=="good"),
       label = TRUE,
       label_alpha = TRUE,
       name = "") +
  ggplot2::theme(legend.position = "bottom")
{r echo=FALSE,include= FALSE, Multivariate_Plots_3}
column_names = colnames(redwines)
r1 <- ggpairs(data=subset(redwines, quality.cat=="bad"), 
             
             #diag=list(continuous="density"),
             lower= list(continuous = wrap("points", alpha = 1/5)),
             #we remove certain columns
             columns= column_names[!(column_names %in% c("X", "quality", "quality.cat"))],
             mapping = ggplot2::aes(color = quality.cat), 
             axisLabels="show") + 
    ggplot2::theme_light(base_size = 10)

r2 <- ggpairs(data=subset(redwines, quality.cat=="medium"),
             lower= list(continuous = wrap("points", alpha = 1/5)),
             #we remove certain columns
             columns= column_names[!(column_names %in% c("X", "quality", "quality.cat"))],
             mapping = ggplot2::aes(color = quality.cat), 
...
r3 <- ggpairs(data=subset(redwines, quality.cat=="good"),
             lower= list(continuous = wrap("points", alpha = 1/5)),
             #we remove certain columns
             columns= column_names[!(column_names %in% c("X", "quality", "quality.cat"))],
             mapping = ggplot2::aes(color = quality.cat), 
