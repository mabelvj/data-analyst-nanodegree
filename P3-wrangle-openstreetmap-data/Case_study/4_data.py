...
SCHEMA = schema.schema # previous schema.schema
...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    #pprint.pprint(element.keys())

    #pprint.pprint(set(element.keys()))
    #pprint.pprint(set(node_attr_fields))

    #print(set(element.keys()) == set(node_attr_fields))
    
    #pprint.pprint(node_attr_fields)
    #pprint.pprint(way_attr_fields)

    #print "CHILDREN"
    #pprint.pprint(list(element)) #get the children

    if element.tag == 'node':
      node_attribs = {key:element.get(key) for key in node_attr_fields}
        #print node_attr_fields
      tags = shape_element_tag(element, NODE_TAGS_FIELDS, problem_chars=PROBLEMCHARS, default_tag_type='regular')
      ...
      way_attribs = {key:element.get(key) for key in way_attr_fields}
      #print node_attr_fields
      way_nodes = shape_element_tag(element, WAY_NODES_FIELDS, problem_chars=PROBLEMCHARS, default_tag_type='regular')
      tags = shape_element_tag(element, WAY_TAGS_FIELDS, problem_chars=PROBLEMCHARS, default_tag_type='regular')
      
      return {'way': way_attribs, 'way_nodes': way_nodes, 'way_tags': tags}

def shape_element_tag(element, tags_attr_fields, problem_chars, default_tag_type):
    """Clean and shape node or way XML element to Python dict"""
    tags = []  # Handle secondary tags the same way for both node and way elements
    ...
    dict_keys = {'k' : 'key', 'v': 'value', 'ref': 'node_id'}
      ...
      if (child.tag == 'nd' and tags_attr_fields== WAY_NODES_FIELDS) or (child.tag == 'way' and tags_attr_fields== WAY_TAGS_FIELDS) or (child.tag == 'tag' and tags_attr_fields== NODE_TAGS_FIELDS):
        ...
        for key in child.keys(): # iterate to parse the values in the children
          if key in dict_keys.keys(): # maybe we have the info but is encoded : 'value' appears and 'v'
            ...
            if (key == 'k' and (child.tag=="tag" or child.tag=="way")) and (len(child.attrib['k'].split(':',1))>=2):
              #special case for tag: ie: addr:postcode
              #and
        ...
        if 'node_id' in tags_attr_fields:
          ...
          counter += 1
          tag_dict['position'] = counter

        tags.append({x: tag_dict[x] for x in tags_attr_fields}) #only return the desired values for the type of node
    #print tags
    return tags
